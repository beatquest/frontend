...
export const Matches = ({
  ...
  const buildMatchRow = (match) => {
    return (
      <tr>
        <td>
          <Link to={`/event/${match.event.id}`}>
            <img src={match.event.image} alt={match.event.image} height={75} />
          </Link>
        </td>
        <td>
          <div>
            <Link to={`/user/${match.p1.id}`}>{match.p1.name}</Link>{' '}
            <Delta delta={match.p1.elo.delta} />
          </div>
          <Link to={`/user/${match.p2.id}`}>{match.p2.name}</Link>{' '}
  ...
  return (
    <div class="matches">
      <h2 class="mx-auto mt-5">{}</h2>
      <table class="table table-dark m-5 mx-auto">
        <thead>
          <tr>
